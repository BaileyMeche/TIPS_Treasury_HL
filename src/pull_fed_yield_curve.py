...
def _cutoff_date(years: int, today: datetime | None = None) -> pd.Timestamp:
    """Return the earliest date to retain.

    Parameters
    ----------
    years
        Number of calendar years of history to preserve.
    today
        Reference date used when computing the cutoff. Defaults to the
        current day which makes the function easy to test by injecting a
        deterministic value.
    """
...
def pull_fed_yield_curve(years: int = 3, today: datetime | None = None):
    """
    ...
    df_all = pd.read_csv(pdf_stream, skiprows=9, index_col=0)
    df_all.index = pd.to_datetime(df_all.index)

    cutoff = _cutoff_date(years, today)
    df_all = df_all[df_all.index >= cutoff].copy()
